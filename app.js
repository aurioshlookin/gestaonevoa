...
const App = () => {
    ...
    const [accessConfig, setAccessConfig] = useState({
        kamiRoleId: '', councilRoleId: '', moderatorRoleId: '', creatorId: '', vipIds: []
    });
    ...
    const [newMember, setNewMember] = useState({ 
        name: '', discordId: '', role: '', specificRoleId: '', ninRole: '', 
        isLeader: false, joinDate: new Date().toISOString().split('T')[0]
    });
    ...
    const getUserRoleLabel = useMemo(() => {
        ...
        Object.entries(roleConfig).forEach(([orgId, roleId]) => {
            if (user.roles.includes(roleId) && (!leaderRoleConfig[orgId] || !user.roles.includes(leaderRoleConfig[orgId]))) roles.push(`Membro ${ORG_CONFIG[orgId]?.name || ''}`);
        });
    ...
    const handleAddVip = async () => {
        ...
        try {
            ...
            setVipSelection(""); showNotification('VIP Adicionado!', 'success');
    ...
    const openCreateModal = () => {
        setIsCreating(true); setSearchTerm("");
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            showNotification('Salvo com sucesso!', 'success'); setSelectedMember(null); setEditForm(null);
    ...
    const handleRemoveMember = async (id) => {
        ...
        try {
            ...
            showNotification('Removido com sucesso.', 'success'); setDeleteConfirmation(null);
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            await setDoc(doc(db, "server", "config"), { roleMapping: roleConfig, leaderRoleMapping: leaderRoleConfig, secLeaderRoleMapping: secLeaderRoleConfig, accessConfig: accessConfig }, { merge: true });
            logAction(user, "Configurações", "Sistema", "Cargos e Permissões atualizados");
            showNotification('Configurações Salvas!', 'success'); setShowSettings(false);
    ...
    const handleSelectUser = (user) => {
        ...
        setSearchTerm(user.displayName || user.username); setIsDropdownOpen(false);
    };
    ...
    useEffect(() => { 
        const f = new URLSearchParams(window.location.hash.slice(1)); let accessToken = f.get('access_token') || localStorage.getItem('discord_access_token');
        if (f.get('access_token')) { localStorage.setItem('discord_access_token', accessToken); window.history.pushState({}, document.title, window.location.pathname); }
        if (accessToken) { 
            setLoading(true);
            fetch('https://discord.com/api/users/@me', {headers:{Authorization:`Bearer ${accessToken}`}}).then(r => { if (!r.ok) throw new Error("Token inválido"); return r.json(); })
            .then(userData => {
                fetch(`https://discord.com/api/users/@me/guilds/${GUILD_ID}/member`, {headers:{Authorization:`Bearer ${accessToken}`}}).then(r => r.ok ? r.json() : { roles: [] }).then(memberData => { setUser({ ...userData, roles: memberData.roles || [] }); setLoading(false); });
            }).catch(err => { console.error(err); localStorage.removeItem('discord_access_token'); setUser(null); setLoading(false); });
    ...
    return (
        <div className="min-h-screen bg-slate-900 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] text-slate-200 font-mono">
            {selectedMember && editForm && <EditMemberModal isCreating={isCreating} editForm={editForm} setEditForm={setEditForm} selectedMember={selectedMember} setSelectedMember={setSelectedMember} handleSaveEdit={handleSaveEdit} discordRoles={discordRoles} canManageOrg={canManageOrg} filteredRoster={filteredRoster} searchTerm={searchTerm} setSearchTerm={setSearchTerm} isDropdownOpen={isDropdownOpen} setIsDropdownOpen={setIsDropdownOpen} handleSelectUser={handleSelectUser} />}
            {deleteConfirmation && <DeleteModal setDeleteConfirmation={setDeleteConfirmation} handleRemoveMember={handleRemoveMember} id={deleteConfirmation} />}
            {showSettings && <SettingsModal setShowSettings={setShowSettings} settingsTab={settingsTab} setSettingsTab={setSettingsTab} roleConfig={roleConfig} setRoleConfig={setRoleConfig} leaderRoleConfig={leaderRoleConfig} setLeaderRoleConfig={setLeaderRoleConfig} secLeaderRoleConfig={secLeaderRoleConfig} setSecLeaderRoleConfig={setSecLeaderRoleConfig} accessConfig={accessConfig} setAccessConfig={setAccessConfig} discordRoles={discordRoles} discordRoster={discordRoster} vipSelection={vipSelection} setVipSelection={setVipSelection} handleAddVip={handleAddVip} handleRemoveVip={handleRemoveVip} handleSaveConfig={handleSaveConfig} canManageSettings={canManageSettings} Icons={Icons} />}
            ...
            <header className="border-b border-slate-800 bg-slate-900/90 backdrop-blur sticky top-0 z-50">
                <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl md:text-2xl font-bold mist-title text-cyan-50 tracking-wider">Gestão da <span className="text-cyan-500">Névoa</span></h1>
                    </div>
                    <div className="flex items-center gap-4">
                        {canViewHistory && (
                            <>
                                <button onClick={() => setActiveTab('access')} className={`p-2 transition-colors ${activeTab === 'access' ? 'text-green-400 bg-green-400/10 rounded-lg' : 'text-slate-400 hover:text-green-400'}`}><Globe size={20} /></button>
                                <button onClick={() => setActiveTab('history')} className={`p-2 transition-colors ${activeTab === 'history' ? 'text-purple-400 bg-purple-400/10 rounded-lg' : 'text-slate-400 hover:text-purple-400'}`}><History size={20} /></button>
                            </>
                        )}
                        {canManageSettings && <button onClick={() => setShowSettings(true)} className="p-2 text-slate-400 hover:text-cyan-400 transition-colors"><Settings size={20} /></button>}
                ...
                {activeTab === 'dashboard' ? (
                    ...
                    <div className="animate-fade-in">
                        ...
                        {ORG_CONFIG[activeTab].roleDetails && (
                            <div className="mb-6">
                                <button onClick={() => setShowRoleDetails(!showRoleDetails)} className="w-full bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded-lg p-3 flex justify-between items-center transition-all text-slate-300 hover:text-white"><span className="font-bold flex items-center gap-2"><BookOpen size={18}/> Hierarquia & Cargos</span>{showRoleDetails ? <ChevronUp size={20}/> : <ChevronDown size={20}/>}</button>
                                {showRoleDetails && (<div className="mt-2 bg-slate-900/50 border border-slate-700 rounded-lg p-4 animate-fade-in grid grid-cols-1 md:grid-cols-2 gap-4">{ORG_CONFIG[activeTab].roleDetails.map((role, idx) => (<div key={idx} className="flex flex-col border-l-2 border-slate-600 pl-3"><span className="text-sm font-bold text-cyan-400">{role.name}</span><span className="text-xs text-slate-400">{role.desc}</span></div>))}</div>)}
                            </div>
                        )}
                        {canManageOrg(activeTab) && <div className="mb-6 flex justify-end"><button onClick={openCreateModal} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-6 rounded-lg flex items-center gap-2 shadow-lg shadow-cyan-500/20 transition-all hover:scale-105"><UserPlus size={20} /> Adicionar Novo Membro</button></div>}
                        <div className="glass-panel rounded-xl overflow-hidden border border-slate-700">
                            <table className="w-full text-left">
                                <thead className="bg-slate-800/50 text-slate-400 text-xs uppercase"><tr><th className="p-4 cursor-pointer hover:text-white" onClick={() => requestSort('role')}>Cargo Discord <SortIcon k="role" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th><th className="p-4 cursor-pointer hover:text-white" onClick={() => requestSort('name')}>Nome <SortIcon k="name" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th><th className="p-4 cursor-pointer hover:text-white" onClick={() => requestSort('ninRole')}>Cargo Nin <SortIcon k="ninRole" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th><th className="p-4 cursor-pointer hover:text-white" onClick={() => requestSort('joinDate')}>Entrada <SortIcon k="joinDate" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th><th className="p-4 cursor-pointer hover:text-white" onClick={() => requestSort('activity')}>Atividade <SortIcon k="activity" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th><th className="p-4 text-center cursor-pointer hover:text-white" onClick={() => requestSort('isLeader')}>Líder <SortIcon k="isLeader" sortConfig={sortConfig} ArrowUp={ArrowUp} ArrowDown={ArrowDown} ArrowUpDown={ArrowUpDown}/></th>{canManageOrg(activeTab) && <th className="p-4 text-right">Ações</th>}</tr></thead>
                                <tbody className="divide-y divide-slate-700">
                                    {getSortedMembers(members.filter(m => m.org === activeTab)).map((member) => {
                                        ...
                                        return (
                                            <tr key={member.id} className={`hover:bg-slate-800/30 transition-colors cursor-pointer ${member.isLeader ? 'bg-yellow-900/10' : ''}`} onClick={() => { if(canManageOrg(activeTab)) openEditModal(member); }}>
                                                ...
                                                <td className="p-4"><div className="flex flex-col"><span className="font-bold text-white flex items-center gap-2">{member.name}{orgInfo && (<div className="text-yellow-400 cursor-help relative group" title={`Membro de: ${orgInfo.names}`} onClick={(e) => e.stopPropagation()}><AlertCircle size={14} /></div>)}</span><div className="flex flex-wrap gap-2 mt-1">{(member.masteries||[]).map(m => { const mData = MASTERIES.find(mastery => mastery.id === m); if (!mData) return null; return (<div key={m} className={`flex items-center gap-1 ${mData.color} bg-slate-800/50 px-1.5 py-0.5 rounded text-[10px] font-bold border border-${mData.color.split('-')[1]}-500/20`}>{React.createElement(mData.icon, {size: 12})}<span>{m}</span></div>) })}</div></div></td>
