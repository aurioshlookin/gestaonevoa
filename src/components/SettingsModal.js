...
import { Settings, ShieldCheck, UserCog, Star, Trash2 } from 'lucide-react';
import { ORG_CONFIG, Icons } from '../config/constants.js';

const SettingsModal = ({ 
    roleConfig, leaderRoleConfig, secLeaderRoleConfig, accessConfig, 
    discordRoles, discordRoster, onClose, onSave, canManageSettings 
}) => {
        ...
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-800 border border-slate-600 rounded-xl w-full max-w-2xl shadow-2xl animate-fade-in flex flex-col max-h-[90vh]">
                    ...
                    {activeTab === 'roles' && (
                        <div className="space-y-6">
                            {Object.values(ORG_CONFIG).map(org => {
                                const IconComp = Icons[org.icon] || Icons.Activity;
                                return (
                                    <div key={org.id} className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                                        <h3 className={`text-sm uppercase font-bold ${org.color} mb-3 flex items-center gap-2`}>
                                            {React.createElement(IconComp, {size: 16})} {org.name}
                                        </h3>
                    ...
                    {activeTab === 'permissions' && (
                        <div className="space-y-6">
                            <div className="bg-yellow-900/10 border border-yellow-600/30 p-4 rounded-lg mb-6">
                                ...
                                <p className="text-xs text-slate-400">Defina cargos de gestão.</p>
                            </div>
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                                        <label className="text-sm font-bold text-red-400 mb-2 block">Kage</label>
                                        <select className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white outline-none" value={localAccessConfig.kamiRoleId || ""} onChange={(e) => setLocalAccessConfig({...localAccessConfig, kamiRoleId: e.target.value})}>
                                            <option value="">Selecione...</option>
                                            {discordRoles.map(r => <option key={r.id} value={r.id} style={{color: r.color}}>{r.name}</option>)}
                                        </select>
                                    </div>
                                    <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                                        <label className="text-sm font-bold text-orange-400 mb-2 block">Conselho</label>
                                        <select className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white outline-none" value={localAccessConfig.councilRoleId || ""} onChange={(e) => setLocalAccessConfig({...localAccessConfig, councilRoleId: e.target.value})}>
                                            <option value="">Selecione...</option>
                                            {discordRoles.map(r => <option key={r.id} value={r.id} style={{color: r.color}}>{r.name}</option>)}
                                        </select>
                                    </div>
                                ...
                                <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                                    <label className="text-sm font-bold text-blue-400 mb-2 block">Moderador</label>
                                    <select className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white outline-none" value={localAccessConfig.moderatorRoleId || ""} onChange={(e) => setLocalAccessConfig({...localAccessConfig, moderatorRoleId: e.target.value})}>
                                        <option value="">Selecione...</option>
                                        {discordRoles.map(r => <option key={r.id} value={r.id} style={{color: r.color}}>{r.name}</option>)}
                                    </select>
                                </div>
                ...
                <div className="p-6 border-t border-slate-700 bg-slate-900/50 rounded-b-xl flex justify-between items-center">
                    <span className="text-xs text-slate-500">Alterações salvam ao clicar.</span>
